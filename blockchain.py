...
class Blockchain:
            ...
            with open('blockchain-{}.txt'.format(self.node_id), mode='r') as f:
                ...
                for block in blockchain:
                    converted_tx = [Transaction(
                        tx['official'], tx['image'], tx['imageText'], tx['signature']) for tx in block['transactions']]
                    updated_block = Block(
                ...
                for tx in open_transactions:
                    updated_transaction = Transaction(
                        tx['official'], tx['image'], tx['imageText'], tx['signature'])
    ...
    def add_transaction(self, official,image,imageText , signature, is_receiving=False):
        if Verification.verify_chain(self.chain)==False:
            return False

        transaction = Transaction(official,image,imageText , signature)
            ...
            for node in self.__peer_nodes:
                ...
                try:
                    response = requests.post(url, json={
                                             'official': official, 'image': image, 'imageText': imageText, 'signature': signature})
    ...
    def mine_block(self,is_receiving=False):
        if Verification.verify_chain(self.chain)==False:
            return False
        ...
        for tx in copied_transactions:
            if not Desk.verify_transaction(tx):
                return None
    ...
    def add_block(self, block):
        ...
        for itx in block['transactions']:
            for opentx in stored_transactions:
                if opentx.official == itx['official'] and opentx.image == itx['image'] and opentx.imageText == itx['imageText'] and opentx.signature == itx['signature']:
                    try:
